...
export async function storeImageApi(image?: ImageToStore | null): Promise<Image | undefined> {
  ...
  const response = await $http.post('api/image', { image }, { useFormData: true });
  if (response.success) {
...
export async function saveImageApi({ imageToSave, name, folder = 'unspecified', apiPath = apiPathDefaults }: SaveImageApiParams): Promise<Image | undefined> {
  if (!imageToSave) return undefined;
  const response = await $http.post(apiPath?.savePath, { image: imageToSave }, { useFormData: true });
  if (response.success) {
...
export async function removeImageApi({ image, name, folder = 'unspecified', apiPath = apiPathDefaults }: RemoveImageApiParams): Promise<Image | undefined> {
  if (!image) return undefined;
